...
const ai_bridge_1 = require("./ai-bridge");
...
async function collectFiles(rootDir, extFilter) {
    const files = [];
    for await (const f of (0, walker_1.walkDir)(rootDir, { includeExtensions: extFilter })) {
        files.push(f);
    }
    return files;
}
// ── Commands ───────────────────────────────────────────────────────────────
/** vela init [dir] */
function cmdInit(targets) {
    const rootDir = args[1] ? path.resolve(args[1]) : process.cwd();
    const store = (0, store_1.initStore)(rootDir, targets);
    (0, ai_bridge_1.writeClaudeMd)(rootDir, store);
...
async function cmdSnap() {
    ...
    const store = (0, store_1.loadStore)(rootDir);
    let filePaths;
    if (fileArg) {
    ...
    const checkpoint = (0, store_1.createCheckpoint)(rootDir, store, { intent, description, filePaths, tags });
    (0, ai_bridge_1.writeClaudeMd)(rootDir, store);
...
function cmdLog() {
    const rootDir = args[1] ? path.resolve(args[1]) : process.cwd();
    const store = (0, store_1.loadStore)(rootDir);
    const history = (0, store_1.getHistory)(store);
    if (history.length === 0) {
    ...
    history.forEach((cp, i) => {
        ...
        if (cp.tags.length)
            console.log(`  ${cp.tags.map(t => color('35', `#${t}`)).join(' ')}`);
        console.log('');
    });
}
...
function cmdRestore() {
    ...
    const store = (0, store_1.loadStore)(rootDir);
    const checkpoint = (0, store_1.resolveCheckpoint)(store, ref);
    if (!checkpoint) {
        console.error(`✗ No checkpoint matching "${ref}"`);
        console.error('Run `vela log` to see available codenames.');
        process.exit(1);
    }
    const plan = (0, ai_bridge_1.generateRollbackPlan)(checkpoint, rootDir);
    ...
    if (!flag('dry-run')) {
        const restored = (0, store_1.restoreCheckpoint)(checkpoint, rootDir);
        // Snap the post-restore state as a new checkpoint so history stays linear
        const store2 = (0, store_1.loadStore)(rootDir);
        (0, store_1.createCheckpoint)(rootDir, store2, {
            intent: `rollback to: ${checkpoint.intent}`,
            description: `Rolled back to checkpoint ${checkpoint.codename}`,
            filePaths: restored,
            tags: ['rollback'],
        });
        (0, ai_bridge_1.writeClaudeMd)(rootDir, store2);
...
function cmdPlan() {
    ...
    const rootDir = args[2] ? path.resolve(args[2]) : process.cwd();
    if (!ref) {
        console.error('Usage: vela plan <codename-or-id> [dir]');
        process.exit(1);
    }
    const store = (0, store_1.loadStore)(rootDir);
    const checkpoint = (0, store_1.resolveCheckpoint)(store, ref);
...
async function cmdWatch() {
    ...
    const store = (0, store_1.initStore)(rootDir);
    const { CodebaseWatcher } = await Promise.resolve().then(() => __importStar(require('./watcher')));
    const watcher = new CodebaseWatcher(rootDir, {
        ...
        onChange: async (filePath, type) => {
            if (type === 'change') {
                ...
                const s = (0, store_1.loadStore)(rootDir);
                (0, store_1.createCheckpoint)(rootDir, s, {
                    intent: `auto: ${intent}`,
                    description: `Auto-snapshot of ${path.relative(rootDir, filePath)}`,
                    filePaths: [filePath],
                    tags: ['auto', 'watch'],
                });
                (0, ai_bridge_1.writeClaudeMd)(rootDir, (0, store_1.loadStore)(rootDir));
                const cp = (0, store_1.loadStore)(rootDir).checkpoints[(0, store_1.loadStore)(rootDir).checkpoints.length - 1];
...
function cmdCommitPurge() {
    ...
    if (!store.head) {
        if (!isHook)
            console.log('No checkpoints to purge.');
        return;
    }
    const cycle = (0, store_1.getHistory)(store);
    if (cycle.length === 0) {
        if (!isHook)
    ...
    // Get git commit info
    const git = (0, store_1.getLatestGitCommit)(rootDir);
    if (!git) {
        console.error('✗ Could not read git commit. Are you in a git repo?');
        process.exit(1);
    }
    const result = (0, store_1.purgeAfterCommit)(rootDir, store, git.hash, git.message);
    (0, ai_bridge_1.writeClaudeMd)(rootDir, (0, store_1.loadStore)(rootDir));
    if (isHook) {
...
function cmdHook() {
    const rootDir = args[1] && !args[1].startsWith('--') ? path.resolve(args[1]) : process.cwd();
    try {
        const hookPath = (0, store_1.installGitHook)(rootDir);
...
function cmdDiff() {
    ...
    console.log((0, diff_1.renderDiff)(sessionDiff, { color: !flag('no-color') }));
}
switch (command) {
    case 'init':
        cmdInit();
    ...
    case 'codex':
        ...
        break;
    case 'snap':
        cmdSnap().catch(e => { console.error(e.message); process.exit(1); });
        break;
    case 'log':
        cmdLog();
        break;
    ...
    case 'plan':
        ...
        break;
    case 'watch':
        cmdWatch().catch(e => { console.error(e.message); process.exit(1); });
        break;
    case 'commit-purge':
        cmdCommitPurge();
        break;
    ...
    case 'scan':
        (async () => {
            ...
            const stats = await (0, index_1.scanCodebase)(rootDir, {
